...
cp -arf /srv/***REMOVED***.bak/. /srv/***REMOVED***

# restore apache vhosts
cp -rf new_vhost_confs/. /etc/apache2/sites-available
for vhost in `ls new_vhost_confs`; do ln -sf /etc/apache2/sites-available/$vhost /etc/apache2/sites-enabled/$vhost; done
...
apt-get install php5-{cli,common,curl,dev,gd,imap,mcrypt,memcache,mysql,xsl,intl,fpm} -y
...
# Install apache fast***REMOVED***i
apt-get install libapache2-mod-fast***REMOVED***i -y
...
# restore modified crons and scheduler file (with reference to php52 binary)
cp -rf /home/***REMOVED***/phpbinfix/crond/. /etc/cron.d
cp -f /home/***REMOVED***/phpbinfix/SchedulerLinuxLauncher.php /srv/***REMOVED***/lib/php5/***REMOVED***/projects/intranet/***REMOVED***/offspring/scheduler/
...
sed -i '11i Defaults   secure_path="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"' /etc/sudoers
