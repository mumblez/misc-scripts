...
EMAILS="someone1@company.com someone2@company.com"
...
if ! tail -n 1 $FS_TMP_LOG | grep -q 'Updating completed.'; then
	cat $FS_TMP_LOG | mail -s "FactSet LiveData sync failure" it-monitoring@company.com
else
	if ! tail -n -2 $FS_TMP_LOG | head -n 1 | grep -q 'Nothing to update'; then
		cat $FS_TMP_LOG | mail -aFrom:admin@company.com -s "FactSet LiveData updated with changes" $EMAILS
	fi
fi
