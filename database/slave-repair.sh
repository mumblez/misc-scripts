...
MAX_RSYNC_THREADS=8 # make configurable in rundeck
EXCLUDE_LIST="services service_configuration scheduled_task"
START_TIME=$(date)
MYSQL_MASTER_INFO_FILE="/tmp/mysqlMasterInfoStuff.txt"
...
mysql --defaults-file=/***REMOVED***/.my.cnf.backup -h "$REMOTE_DB_SERVER" << EOF > $MYSQL_MASTER_INFO_FILE
STOP SLAVE;
...
MASTER_BIN_FILE=$(grep 'mysql-bin' "$MYSQL_MASTER_INFO_FILE" | awk '{print $1}')
MASTER_POS=$(grep 'mysql-bin' "$MYSQL_MASTER_INFO_FILE" | awk '{print $2}')
mysql << EOF
STOP SLAVE;
CHANGE MASTER TO MASTER_HOST='$REMOTE_DB_SERVER', MASTER_USER='slave_user', MASTER_PASSWORD='***REMOVED***', MASTER_LOG_FILE='$MASTER_BIN_FILE', MASTER_LOG_POS=$MASTER_POS;
SHOW SLAVE STATUS;
quit
EOF
echo "============================================="
