...
{
	echo $* 1>&2
#	echo "$*" | mail -s "Remote Backup Issue: $*" someone@company.com
	exit 1
}
...
ZB_JOBS="/root/scripts/zbackup-remote-backup-jobs.csv"
#ZB_JOBS="/root/scripts/debug.csv"
...
do
	if [ "$DEBUG" = "TRUE" ];
	then
		SSH_USER="some.one" # (requires Defaults:someone !requiretty on destination sudoers config)
		ZB_JOBS="/root/scripts/test.job"
	fi
